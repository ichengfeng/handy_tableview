...
class HandyTableView extends StatefulWidget {
  ...
  final WidgetBuilder? tableViewHeaderBuilder; //tableviewHeaderView builder方法
  final WidgetBuilder? tableViewFooterBuilder; //tableviewFooterView builder方法
...
class _HandyTableViewState extends State<HandyTableView> {
  ...
  void _initData() {
    ...
    final sections = widget.numberOfSections+2;
    for (int section = 0; section < sections; section++) {
      //遍历section
      int rowCount = 0; //获取每个section(区)有多少个row
      ...
      //ListView itemCount总行数（row+header+footer+tableHeaderView+tableFooterView）
      totalCount = totalCount + rowCount + 1 + 1 + 2;
  ...
  Widget _itemBuilder(BuildContext context, int index) {
    ...
    if (model is SectionHeaderModel) {
      ...
      if (model.indexPath.section == 0){
        item = widget.tableViewHeaderBuilder != null
            ? widget.tableViewHeaderBuilder!(context)
            : Container();
      }else if (model.indexPath.section == sectionList.length-1) {
        item = widget.tableViewFooterBuilder != null
